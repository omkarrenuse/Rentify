...
export class AdminService {
  ...
  async deleteVehicle(id: Types.ObjectId) {
    try {
      const deletedVehicle = await this.vehiclesModel.updateOne({ _id: id }, { $set: { is_deleted: true } }).exec();
    }catch{
      throw new Error('Vehicle Deletion interrupted')
    }
  }
